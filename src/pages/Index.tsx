      ...
      <img src={`/screenshots/${id}.png`} alt={label} className="w-full"
        onError={(e) => {
          ...
          if (p && !p.querySelector('.ph')) {
            ...
            d.innerHTML = `<div style="width:48px;height:48px;border-radius:8px;border:1px dashed rgba(0,255,136,0.2);display:flex;align-items:center;justify-content:center;margin-bottom:12px"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(0,255,136,0.3)" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg></div><div style="font-family:Oswald;font-size:11px;letter-spacing:0.15em;text-transform:uppercase;color:rgba(255,255,255,0.2);text-align:center">${label}</div><div style="font-size:10px;color:rgba(255,255,255,0.12);margin-top:4px;text-align:center">Файл: /screenshots/${id}.png</div>`;
            p.appendChild(d);
          }
        }}
      />
      <div className="px-4 py-3" style={{ background: 'rgba(0,0,0,0.5)', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
        <div className="text-xs font-oswald tracking-wider uppercase text-white/45">{label}</div>
        <div className="text-xs text-white/20 mt-0.5">{sublabel}</div>
      </div>
    </div>
  </Reveal>
);
...
const MidCTA = () => (
  <Reveal>
    <div className="my-12 text-center py-8 rounded-xl relative overflow-hidden"
    ...
    <div className="min-h-screen bg-background text-foreground font-golos overflow-x-hidden noise-overlay">
      ...
      <div className="max-w-2xl mx-auto px-4 relative z-10">
        ...
        <section id="цифры" className="pt-16 md:pt-24 pb-6 section-glow-green">
          ...
          <Reveal><P>Суммарно с февраля: <W>$1,875</W>. Одна модель, я один, аккаунт только раскачивается.</P></Reveal>
          <Reveal><P>Сейчас покажу как это устроено. Без теории — моя реальная кухня. Сколько стоит, сколько времени занимает и где большинство людей теряют деньги.</P></Reveal>
        </section>

        {/* ЧТО ЭТО ТАКОЕ */}
        <SectionTitle title="Что это такое" sub="Рынок на $24 миллиарда — и в нём почти нет русскоязычных" />
        <Reveal><P>Есть направление которое растёт с бешеной скоростью. <W>Виртуальные инфлюенсеры.</W></P></Reveal>
        <Reveal><P>Суть: нейросеть создаёт уникального персонажа. Лицо, которое не принадлежит ни одному реальному человеку. Дальше эта же нейросеть делает фотосессии в любых стилях, видео, контент. Ты публикуешь его в TikTok и Instagram. Аудитория растёт.</P></Reveal>
        <Reveal><div className="screenshot-frame my-8 rounded-lg overflow-hidden"><img src={PROCESS_IMG} alt="" className="w-full" /><div className="px-4 py-3" style={{ background: 'rgba(0,0,0,0.5)', borderTop: '1px solid rgba(255,255,255,0.04)' }}><div className="text-xs font-oswald tracking-wider uppercase text-white/40">Процесс создания AI-модели</div><div className="text-xs text-white/20 mt-0.5">Нейросеть генерирует уникальное лицо и контент</div></div></div></Reveal>
          ...
          <div className="rounded-xl p-6 mb-6" style={{ background: 'rgba(0,255,136,0.03)', border: '1px solid rgba(0,255,136,0.1)' }}>
            <div className="space-y-4 text-sm">
              {[
                ...
                { icon: "TrendingUp", text: <>Emily Pellegrini — <W>$23,000 за один месяц</W> на Fanvue. Рост с $6,000 за 3 месяца</> },
              ].map((item, i) => (
        ...
        <Reveal><P accent>Дальше — монетизация:</P></Reveal>
        <Reveal>
        ...
        <Reveal><P accent>Тот, у кого есть органический трафик, в деньгах не нуждается. Виртуальный инфлюенсер — это машина которая генерирует этот трафик. На любую аудиторию мира.</P></Reveal>
        <Reveal><P>И это легально. Ты создаёшь полностью вымышленного персонажа. Ничьё настоящее лицо не используется. Площадки знают про AI-контент и имеют для него правила.</P></Reveal>

        {/* КАК Я НА ЭТО ВЫШЕЛ */}
        <SectionTitle title="Как я на это вышел" sub="Автоматизация — моя основная тема" />
        <Reveal><P>Я занимаюсь автоматизацией и AI-инструментами. Строю системы которые работают без постоянного участия человека.</P></Reveal>
        ...
        <Reveal><P>Хотя сама генерация — не проблема. За пару часов на облачном сервере можно наделать контента на месяц. Настоящая проблема — понять <W>ЧТО</W> генерировать. Какой контент зайдёт, что сейчас в тренде.</P></Reveal>
        <MidCTA />

        {/* ТРИ ОШИБКИ */}
        <SectionTitle title="Три ошибки" sub="На которые я потратил 2 месяца и $100" />
        <Reveal><P>Не буду делать вид что сразу полетело. <W>Первый месяц — $40.</W></P></Reveal>
        <Reveal><P>Почему так мало? Три конкретных ошибки.</P></Reveal>
        <div className="space-y-3 my-6">
          {[
            { title: "Ошибка 1: не знал как прогревать аккаунт", text: "Первый TikTok — бан через 5 дней. Второй — бан через неделю. Соцсети банят новые аккаунты если сразу начинаешь активно постить. Потерял 2 аккаунта и $100 на контент." },
        ...
        <Reveal><P>Когда разобрался во всём на своих ошибках — упаковал это в закрытый клуб.</P></Reveal>
        <Reveal><P accent>Не курс с видеоуроками которые посмотришь и забудешь. А закрытое сообщество в Telegram с готовой системой. Зашёл — и с первого раздела начал делать. Всё выстроено по шагам.</P></Reveal>
        ...
        <Reveal><div className="text-xs font-oswald tracking-[0.15em] uppercase text-white/25 mb-8">Клуб AI Models Factory. 35 человек — от программистов до бухгалтеров, от Москвы до Берлина.</div></Reveal>
        <Reveal><div className="text-white/80 text-sm font-medium mb-5">Что внутри:</div></Reveal>
        <div className="space-y-3 mb-6">
          {[
        ...
        <Reveal>
          <div className="rounded-xl overflow-hidden mb-6" style={{ border: '1px solid rgba(255,255,255,0.05)' }}>
            <div className="grid grid-cols-4 text-xs font-oswald tracking-wider uppercase px-4 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(255,255,255,0.015)' }}>
        ...
        {/* FAQ */}
        <SectionTitle title="Частые вопросы" sub="Если остались сомнения" />
        <Reveal>
          <div className="mb-8">
            {[
        ...
        {/* ЗАНЯТЬ МЕСТО */}
        <section id="занять-место" className="py-16 md:py-24 section-glow-green">
          ...
          <Reveal delay={100}>
            {sent ? (
              <div className="rounded-xl p-10 text-center" style={{ border: '1px solid rgba(0,255,136,0.25)', background: 'rgba(0,255,136,0.03)' }}>
                ...
                <p className="text-white/40 text-sm">Напишу в Telegram в ближайшее время. Отвечу на вопросы и помогу с оплатой.</p>
              </div>
            ) : (
